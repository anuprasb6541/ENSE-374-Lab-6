	...
	*/
	public int getMainMenuNumber()
    {
        ...
        while(numericAnswer == 0)
        {
			try{
				...
				System.out.println("Input is not an integer value. Please try again.");
				numericAnswer = getInt(mainMenuString);
			}
            if (numericAnswer == 0)
            {
	...
	private int getInt(String prompt)
    {
		...
		try{
			...
			System.out.println("Input is not an integer value. Please try again.");
			in.nextLine(); intAnswer = getInt(prompt);
		}
        return intAnswer;
    }
    ...
    private float getFloat(String prompt)
    {
		...
		try{
			...
			System.out.println("Input is not a floating value. Please try again.");
			in.nextLine();
